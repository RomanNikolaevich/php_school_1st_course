...
для того, чтобы вывести большой код html нам не обязательно писать echo '' (где внутри кавычек писать html код на много строчек). Достаточно закрыть php и дальше писать html, а в конце открыть php и закончили наше условие, как в примере выше <?php } ?> -->
...
//База данных взаимодействует с каждым отдельным пользователем и позволяет пользователям выводить конкретные данные. Базы данных будем изучать позже.
...
// Создаем базу данных в вкладке - "База данных". Задаем кодировку UTF-8: 'utf8_unicode_ci'. Жмем кнопку "создать"

//Подключаем определенных людей к этой базе данных с указанием логина и пароля (авторизация). Для этог вкладка "Учетные записи пользователей" => кнопка "Добавить пользователя".
...
// Чтобы отделять одинаковые сообщения одни от других в ID пользователя добавить счетчик просмотров и тогда в базе не будет полностью одинаковых записей. Для этого в настройках в разделе в первой строчке там где прописываем Имя" пишем "id" дальше в колонке "Индекс" выбираем "PRIMARY"

18-6-2 //Тип выбираем для "id" выбираем "INT", для "name" - "TEXT", для "text" - "TEXT", для "date" - "DATE".
//Есть еще более расширенные варианты числовых данных, даты и прочего. Например для чисел - от выбора типа зависит байтовый размер типа (чем больше прежполагается будет база, тем больше нужно выбирать тип):
...
//тут можно задать например если выбрали тип DATE в вкладке "по умолчанию" выбрать "как определено" и прописать вручную формат даты и времени: "2013-01-01 12:12:12". Или с помощью "как определено" можно прописать логин по умолчанию от которого будет писаться коментарий, если не выбран другой, например "guest"

18-6-5 // Сравнение - это наша кодировка, задаем если нужно задать кодировку отличную от предыдущих настроек.


18-6-6 // A_I:  Атрибут AUTO_INCREMENT может использоваться для генерации уникального идентификатора для новых строк. Если отметить этот параметр в строчке 'id' и в вкладке "Индекс" выбрать "PRIMARY", то 'id' у каждой следующей записи комментариев будет увеличиваться на единицу.
...
18-7// Домашка
//Сделать БД: "Пользователи" - туда внести "имя", "возраст/год рождения", "дата создания своего аккаунта", логин, пароль, краткую информацию о себе. Лучше всего посмотреть по разным сайтам какие поля заполняюься при регистрации.
// для пароля выбираем тип "text" , для пароля - "varchar" или "text"
/*Создать Базу Данных, пользователя и 3 таблицы: comments, users, news. Сохранить структуру таблиц в формате .sql . Кроме этого создать 1 word-файл, в котором написать маленькое описание трёх таблиц, то есть что значат конкретные поля. По примеру: `age` - возраст зарегистрированного пользователя. `text` - сообщение комментария и т.д.
Дополнение к уроку:

Для возраста мы указываем `age` типа TINYINT, содержание: 1 или 0 .

Если таблица называется `users`, то поле мы называем `login`, в общей связке мы получим: `users`.`login`. Ошибкой будет назвать поле `user_login`, в таком случае мы получим `users`.`user_login`, то есть тавтология - повторение дважды слова `user`.
В свою очередь мы будем применять имена из двух слов `user_login` тогда, когда таблица не связана будет прямо с пользователями, допустим это у нас таблица `news`, и мы указываем имя пользователя, который написал данную новость: `news`.`user_login`.*/
